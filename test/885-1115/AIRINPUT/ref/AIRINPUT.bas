...
140 FOR J=0 TO MD
150 REC=(JMOD51)+1:SS=J [FC] 51
160 IF [0xFF][AF](2)<>REC THEN GET #2,REC
170 OPTION #2,SS*5 AS DU$,5 AS ID$
180 ID$(J)=ID$
...
1790 PI=J
1800 IF FD>1 THEN REC=(JMOD5)+1:SS=J [FC] 5:GET#1,REC:GOSUB2090:FD=FD+1:RO=RO+1
1810 IF FD=0 THEN FD=1:REC=(JMOD5)+1:SS=J [FC] 5:GET#1,REC
1820 NEXT J
...
1960 IF ID$(J)=AP$ THEN FD=FD+1:RO=8+FD:REC=(JMOD5)+1:SS=J [FC] 5:GET#1,REC:PI=J:GOSUB2090
1970 NEXT J
...
2050 PI=TI
2060 REC=(PIMOD5)+1:SS=PI [FC] 5:GET#1,REC
2070 GOSUB2090:GOSUB3220
2080 RETURN
2090 :REM decode & display
2100 OPTION #1,SS*50 AS DU$,5 AS ID$,2 AS FAC$,4 AS FR$,20 AS NM$,2 AS D1$,4 AS M1$,2 AS D$,4 AS M$,4 AS V$,1 AS V1$,2 AS EL$
...
3030 Y6=Y4-Y5
3040 Y7=Y6*60
3050 REC=(PIMOD5)+1:SS=PI [FC] 5:GET#1,REC
3060 RO=3:GOSUB2090
3070 LSET D1$=LOF(Y)
3080 LSET M1$=MKI$(Y2)
3090 LSET D$=LOF(Y5)
...
3390 RC=(JMOD51)+1:SS=J [FC] 51
3400 IF REC<>RC THEN REC=RC:PUT#2,[0xFF][AF](2)+1:OPTION#2,255 AS CL$:LSET CL$=" "
3410 OPTION #2,SS*5 AS DU$,5 AS ID$
3420 LSET ID$=ID$(J)
3430 NEXT J
