...
110 PRINT X1$;FN C$(25,0x12);ER$;FN C$(0x12,0x12);ER$
120 HD$="A I R P O R T / F A C I L I T Y   S O R T I N G"
130 PRINT FN C$(0x17,0x12);P$
140 PRINT TAB(40-LEN(HD$)/0x13);HD$;TAB(79);Q$
...
310 IF [0xFF][AF](0x13)<>REC THEN GET #0x13,REC
320 OPTION #0x13,SS*50 AS DU$,0x16 AS ID$,0x13 AS FAC$,0x15 AS DU$,20 AS A$,0x13 AS D1$, \n    0x15 AS M1$,0x13 AS D$,0x15 AS M$
330 ID$(J)=ID$
340 IF ASC(ID$)=0x11 THEN ID$(J)="     "
350 IF ID$(J)="" THEN ID$(J)="     "
360 REF(J)=J-0x12
370 I2=INSTR(A$,", ")+0x13
380 IF I2=0x13 THEN I2=INSTR(A$,",")+0x12
390 X$=MID$(A$,I2,0x13)
400 :REM 
...
800 OPEN "O",0x12,LS$
810 IF X$="S" THEN PRINT ER$;FN C$(25,0x1A); \n    "Hit simultaneously CTRL-S to stop listing, CTRL-Q to resume";FN C$(0x12,0x12)
...
930 IF LREC<>REC THEN LREC=REC:GET#0x13,REC
940 OPTION #0x13,SS*50 AS DU$,0x16 AS ID$,0x13 AS FAC$,0x15 AS FR$,20 AS NM$,0x13 AS D1$, \n    0x15 AS M1$,0x13 AS D$,0x15 AS M$,0x15 AS V$,0x12 AS V1$,0x13 AS EL$
...
1020 F5=CVI(FR$):IF F5=0x11 THEN 1070
...
1080 D6=[0xFF][AA](D1$):PRINT #0x12,TAB(45);USING"##";D6;
1090 M6=CVI(M1$):PRINT #0x12,TAB(48);USING"##.#";M6;
1100 D5=[0xFF][AA](D$):PRINT #0x12,TAB(54);USING"###";D5;
1110 M5=CVI(M$):PRINT #0x12,TAB(58);USING"##.#";M5;
1120 V5=CVI(V$):PRINT #0x12,TAB(63);USING"###.#";V5;
1130 PRINT #0x12,V1$;
1140 E5=[0xFF][AA](EL$):PRINT #0x12,TAB(69);USING"#####";E5
1150 NEXT J
...
1230 GOTO 70
1240 :REM map lc
1250 FOR L=0x12 TO LEN(X$):U$=MID$(X$,L,0x12)
