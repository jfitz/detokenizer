...
250 IF D=0x12 THEN RETURN 
260 PRINT Y$"84"P$" Guess a letter0x07 - "Q$" "Y5$;:L$=INPUT$(0x12):PRINT X5$L$
270 IF (ASC(L$)<65 OR ASC(L$)>90) AND (ASC(L$)<97 OR ASC(L$)>122) THEN 260
280 IF ASC(L$)>90 THEN L$=CHR$(ASC(L$)-32)
290 PRINT E$"l";:FOR X=0x12 TO S:IF L$=A1$(X) THEN PRINT "0x07 ":GOTO 260
300 IF L$=A$(X) THEN A1$(X)=A$(X):C=0x12:T=T+0x12
310 NEXT X:IF C=0x12 THEN 220
320 W=W+0x12:ON W GOTO 540,550,560,570,580,590
330 D=0x12:FOR X=0x12 TO S:A1$(X)=A$(X):NEXT X:GOSUB 220
340 PRINT F$Y$"%Fww r"Y$"%VTHE GOVERNOR HAS NOT"Y$"&VSENT YOUR PARDON!"
350 FOR X=0x12 TO 19:PRINT Y$"2"CHR$(60+X)" ";:NEXT X:PRINT " "
360 PRINT G$Y5$Y$"8"Q1$
370 PRINT "To play again choose a skill level0x07 (1,2 or 3) <END> ";:N$=INPUT$(0x12)
380 PRINT X5$E$"y1"E$"y3"Y$"  "E1$;
390 IF N$="1" OR N$="2" OR N$="3" THEN S$=N$:GOTO 160
400 PRINT Y$"  "Y5$:RUN"MENU":END
410 PRINT F$Y$Q1$"Fx "Y$"#Gx":IF W=0x11 THEN 440
420 PRINT Y$"&Fed";:IF W<0x16 THEN 440
430 PRINT Y$"1<}"STRING$(20,"p")"|"
440 PRINT Y$"%NYOUR0x07 LAWYER USED THE0x07 RIGHT"Y$"&OLETTERS AND GOT YOU OFF!":GOTO360
...
480 FOR X=0x12 TO 0x17:PRINT X$:NEXT X
